...
type DraftData = Partial<VideoProject> & { projectId?: string; projectTitle?: string };
function hydrateFromDraft(project:VideoProject):VideoProject{if(typeof window==="undefined")return project;try{const raw=localStorage.getItem(`${DRAFT_PREFIX}${project.id}`);if(!raw)return project;const d=JSON.parse(raw) as DraftData;if(d.projectId&&d.projectId!==project.id)return project;const merged:Partial<VideoProject>={...project,...d,title:d.projectTitle||project.title,id:project.id};return normalizeProject(merged)||project;}catch{return project;}}
