...
export function readProject(id?:string,title?:string):VideoProject|null{const ps=readProjects();return id?ps.find(p=>p.id===id)||null:title?ps.find(p=>p.title===title)||null:null;}
