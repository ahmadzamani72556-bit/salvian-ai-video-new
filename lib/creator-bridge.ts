...
export function consumeCreatorCredits(amount:number,description:string){return new Promise<{success:boolean;balance:number;message:string}>((resolve,reject)=>{if(typeof window==="undefined")return reject(new Error("Browser context required."));const cost=getVideoCreditCostFromDescription(description,amount);if(!Number.isFinite(cost)||cost<=0)return reject(new Error("Biaya kredit tidak valid."));const requestId=crypto.randomUUID();let settled=false;const handler=(event:MessageEvent)=>{if(event.origin!==CREATOR_ORIGIN||event.data?.type!=="SALVIAN_CREDIT_RESULT"||event.data?.requestId!==requestId)return;settled=true;window.clearTimeout(timer);window.removeEventListener("message",handler);resolve({success:event.data.success===true,balance:Number(event.data.balance||0),message:String(event.data.message||"")});};const timer=window.setTimeout(()=>{if(settled)return;window.removeEventListener("message",handler);reject(new Error("Creator belum merespons. Pastikan akun Creator sudah login."));},15000);window.addEventListener("message",handler);const popup=window.open(CREATOR_CREDIT_BRIDGE_URL,"salvianCreatorCredit","popup,width=440,height=360,resizable=yes");if(!popup){window.clearTimeout(timer);window.removeEventListener("message",handler);reject(new Error("Popup Creator diblokir browser. Izinkan popup untuk SALVIAN AI CREATOR."));return;}window.setTimeout(()=>{if(settled||popup.closed)return;try{popup.postMessage({type:"SALVIAN_CONSUME_CREDITS",requestId,amount:cost,description},CREATOR_ORIGIN);}catch{}},1200);});}

export function refundCreatorCredits(amount:number,description:string){return new Promise<{success:boolean;balance:number;message:string}>((resolve,reject)=>{if(typeof window==="undefined")return reject(new Error("Browser context required."));const cost=getVideoCreditCostFromDescription(description,amount);if(!Number.isFinite(cost)||cost<=0)return reject(new Error("Jumlah refund tidak valid."));const requestId=crypto.randomUUID();let settled=false;const handler=(event:MessageEvent)=>{if(event.origin!==CREATOR_ORIGIN||event.data?.type!=="SALVIAN_REFUND_RESULT"||event.data?.requestId!==requestId)return;settled=true;window.clearTimeout(timer);window.removeEventListener("message",handler);resolve({success:event.data.success===true,balance:Number(event.data.balance||0),message:String(event.data.message||"")});};const timer=window.setTimeout(()=>{if(settled)return;window.removeEventListener("message",handler);reject(new Error("Creator tidak merespons permintaan refund."));},15000);window.addEventListener("message",handler);const popup=window.open(CREATOR_CREDIT_BRIDGE_URL,"salvianCreatorCredit","popup,width=440,height=360,resizable=yes,scrollbars=yes");if(!popup){window.clearTimeout(timer);window.removeEventListener("message",handler);reject(new Error("Popup Creator diblokir browser."));return;}window.setTimeout(()=>{if(settled||popup.closed)return;try{popup.postMessage({type:"SALVIAN_REFUND_CREDITS",requestId,amount:cost,description},CREATOR_ORIGIN);}catch{}},1200);});}
