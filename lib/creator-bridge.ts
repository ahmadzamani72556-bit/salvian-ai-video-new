...
export function isCreatorProfile(value: unknown): value is CreatorProfile { if (!value || typeof value !== "object") return false; const p=value as Record<string,unknown>; return typeof p.display_name === "string" && typeof p.email === "string" && typeof p.plan === "string" && Number.isFinite(Number(p.credits)); }
...
/**
 * Canonical charge gate. Even if an older Video NEW client still sends the
 * retired 225/250/275/300 amount, a long-form production is charged using the
 * current central pricing table based on its duration description.
 */
