...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    if (active) {
      setTitle(active.title); setStatus(active.status); setDuration(`${active.duration} menit`);
    }
  }, []);

  const projectCount = useMemo(() => defaultProjects.length, []);
  const projectId = useMemo(() => makeProjectId(title), [title]);
  ...
  function saveWorkspace() {
    ...
    const project: VideoProject = { id: projectId, title: title.trim() || "Project Video Baru", status, duration: minutes, scenes: [], updatedAt: new Date().toISOString() };
