...
const projects = [
  ...
  { title: "Rahasia Rezeki dalam Kehidupan", status: "Produksi", duration: "8 menit", date: "Kemarin" },
  { title: "Tips Creator Pemula 2026", status: "Siap Render", duration: "5 menit", date: "2 hari lalu" },
];
...
export default function ProjectsPage() {
  ...
  function openProject(project: (typeof projects)[number]) {
    localStorage.setItem("salvian-video-active-project", JSON.stringify(project));
  }
    ...
    <main className="dash-shell">
      ...
      <section className="dash-main">
        ...
        <div className="project-box"><div className="project-list">
          {filtered.map(p=><Link href={`/workspace?project=${encodeURIComponent(p.title)}`} onClick={() => openProject(p)} className="project" key={p.title}>
            <div className="project-icon"><Clapperboard size={18}/></div>
            <div className="project-info"><strong>{p.title}</strong><span>{p.duration} · {p.date}</span></div>
